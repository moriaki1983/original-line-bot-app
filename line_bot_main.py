...
#Postgresデータベース上のテーブル「line_test_entry」の有無を示す変数を宣言する
has_db_table = False
...
def show_db_record():
    ...
    if rcd_id == 0:
       cur.execute("""SELECT * FROM line_test_entry WHERE rcd_id = %(rcd_id)s;""", {'rcd_id': rcd_id})
       rcd = cur.fetchone()
       cur.close()
       conn.close()
       return jsonify(rcd), 200
    if rcd_id >= 1:
       cur.execute("""SELECT * FROM line_test_entry WHERE rcd_id = %(rcd_id)s;""", {'rcd_id': rcd_id - 1})
       rcd = cur.fetchone()
       cur.close()
       conn.close()
       return jsonify(rcd), 200
...
def create_db_table():
    ...
    try:
        cur.execute("""CREATE TABLE line_test_entry(rcd_id int, dttm text, usr_nm text, msg text, intnt text);""")
    except Exception:
        pass
...
def line_msg_analyze(line_msg_txt):
    ...
    if rcd_id >= 4:
       idx = rcd_id - 4
       prv_msgrcd_tmp = cur.execute("""SELECT * FROM line_test_entry WHERE rcd_id = %(idx)s;""", {'idx': idx})
       prv_msgrcd_lst.append([prv_msgrcd_tmp[1], prv_msgrcd_tmp[2], prv_msgrcd_tmp[3], prv_msgrcd_tmp[4]])
       idx = rcd_id - 3
       prv_msgrcd_tmp = cur.execute("""SELECT * FROM line_test_entry WHERE rcd_id = %(idx)s;""", {'idx': idx})
       prv_msgrcd_lst.append([prv_msgrcd_tmp[1], prv_msgrcd_tmp[2], prv_msgrcd_tmp[3], prv_msgrcd_tmp[4]])
...
def postgres_insert_and_update(event, line_msg_intnt):
    ...
    if (rcd_id >= 0 and rcd_id <= 99):
        ...
        if  rcd is not None:
            cur.execute("""UPDATE line_test_entry SET (rcd_id, dttm, usr_nm, msg) VALUES (%(rcd_id)s, %(dttm)s, %(usr_nm)s, %(msg)s, %(intnt)s) WHERE = %(rcd_id)s;""", {'rcd_id': rcd_id, 'dttm' : dttm, 'usr_nm': usr_nm, 'msg': msg, 'intnt':intnt', 'rcd_id': rcd_id})
        rcd_id = rcd_id + 1
    if rcd_id == 100:
       rcd_id = -1
