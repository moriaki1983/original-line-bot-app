...
def generate_line_message():
    ...
    #メッセージの中からトピックを抽出するために新旧５件分のレコードを取得する
    global rcd_id
    if rcd_id == 1:
    ...
    #新旧５件分のメッセージの中からトピックを抽出する
    line_tpc = extract_topic(line_nwmsg, line_oldmsg, line_oldmsg2, line_oldmsg3, line_oldmsg4)

    #新旧のレコードリストとトピックを基にユーザーに返信するLINEメッセージを生成する
    gnrtd_msg = line_bot_text_generate.text_generate_from_analyze_result(line_rcd, line_tpc)
    return gnrtd_msg
