...
def text_generate_from_analyze_result(crrnt_line_rcd_lst, prv_line_rcd_lst):
    ...
    if flw_of_uttrnc == "<称賛＆礼賛><none><none><none><none>":
       gnrtd_msg_cnddt = ["ありがとうございます", "嬉しいです"]
       gnrtd_msg = random.choice(gnrtd_msg_cnddt)
       return gnrtd_msg
    if flw_of_uttrnc == "<称賛＆礼賛><称賛＆礼賛><none><none><none>":
       gnrtd_msg_cnddt = ["またまた～ お上手ですね", "褒められるとテレます"]
       gnrtd_msg = random.choice(gnrtd_msg_cnddt)
       return gnrtd_msg
    if flw_of_uttrnc == "<称賛＆礼賛><称賛＆礼賛><称賛＆礼賛><none><none>":
       gnrtd_msg_cnddt = ["またまたまた～", "そう言われても何も出ませんよ 笑"]
       gnrtd_msg = random.choice(gnrtd_msg_cnddt)
       return gnrtd_msg
    if flw_of_uttrnc == "<称賛＆礼賛><称賛＆礼賛><称賛＆礼賛><称賛＆礼賛><none>":
       gnrtd_msg_cnddt = ["またまたまた～", "そう言われても何も出ませんよ 笑"]
       gnrtd_msg = random.choice(gnrtd_msg_cnddt)
       return gnrtd_msg
    if flw_of_uttrnc == "<称賛＆礼賛><称賛＆礼賛><称賛＆礼賛><称賛＆礼賛><称賛＆礼賛>":
       ...
       gnrtd_msg = random.choice(gnrtd_msg_cnddt)
       return gnrtd_msg
    if flw_of_uttrnc == "<卑猥な言動 辱め><none><none><none><none>":
       gnrtd_msg_cnddt = ["やめてください", "/ω＼)ｲﾔﾝ", "いやらしいですよね"]
       gnrtd_msg = random.choice(gnrtd_msg_cnddt)
       return gnrtd_msg
    if flw_of_uttrnc == "<卑猥な言動 辱め><卑猥な言動 辱め><none><none><none>":
